...
old_routes = re.search(r'@app\.get\("/search", response_class=HTMLResponse\).*?return templates\.TemplateResponse\([^)]+\)', content, re.DOTALL).group(0)
old_browse = re.search(r'@app\.get\("/anime/browse", response_class=HTMLResponse\).*?return templates\.TemplateResponse\([^)]+\)', content, re.DOTALL).group(0)

new_routes = """@app.get("/search", response_class=HTMLResponse)
...
content = content.replace(old_browse, "") # we replaced both with new_routes

with open('app/main.py', 'w') as f:
    f.write(content)

print("Updated search to use advanced AniList query!")
